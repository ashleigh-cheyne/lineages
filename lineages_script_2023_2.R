...
setwd("C:/Users/ashle/OneDrive/Documents/PhD/Data_run")
# .libPaths("C:/Program Files/R/R-3.5.1/library")
# .libPaths("C:/Program Files/R/R-4.3.1/library")
# load("D:/Lineage/lineage_workspace_paper.RData")
...
bork$INFO <- lapply(bork$INFO, rowmodsplit)
# success[[1]][[1]][[1]][[1]]
# remove layers of the list
bork$INFO <- sapply(bork$INFO, `[[`, 1)
bork$INFO <- sapply(bork$INFO, `[`, 1)
# split by |
...
bork$INFO <- lapply(bork$INFO, rowmodsplit2)
bork$INFO <- sapply(bork$INFO, `[[`, 1)
# select the info you want
bork2 <- bork
bork2$INFO <- sapply(bork2$INFO, `[`, 11)
# bork2$INFO <- sapply(bork2$INFO, `[`, 4) # gene name
# select the info you want
bork_snp <- bork
bork_snp$INFO <- sapply(bork_snp$INFO, `[`, 2)

# keep only single nucleotide variants
bork_snp <- subset(bork_snp, bork_snp$REF %in% c("A", "G", "T", "C"))
bork_snp <- subset(bork_snp, bork_snp$ALT %in% c("A", "G", "T", "C"))
# synonymous and non-synonymous as individual objects
success_syn <- subset(bork_snp, bork_snp$INFO == "synonymous_variant")
success_nonsyn <- subset(bork_snp, bork_snp$INFO != "synonymous_variant")

# keeping gene info for the non-synonymous snps
bork_snp2 <- bork
bork_snp2$INFO <- sapply(bork_snp2$INFO, `[`, 4)
bork_snp2 <- subset(bork_snp2, bork_snp2$REF %in% c("A", "G", "T", "C"))
bork_snp2 <- subset(bork_snp2, bork_snp2$ALT %in% c("A", "G", "T", "C"))
bork_snp2 <- subset(bork_snp2, bork_snp2$POS %in% success_nonsyn$POS)
...
# all snps
df1 <- cbind(bork_snp, dummy(bork_snp$Strain, sep = "_"))
df_ordered <- df1[order(df1$POS), ]
colnames(df_ordered) <- gsub("bork_snp_", "s", colnames(df_ordered))
...
bork_nonsyn <- bork
# select only SNP variants
bork_nonsyn$INFO1 <- sapply(bork_nonsyn$INFO, `[`, 2)
bork_nonsyn$INFO2 <- sapply(bork_nonsyn$INFO, `[`, 3)
bork_nonsyn$INFO3 <- sapply(bork_nonsyn$INFO, `[`, 4)
bork_nonsyn$INFO4 <- sapply(bork_nonsyn$INFO, `[`, 11)
bork_nonsyn <- subset(bork_nonsyn, bork_nonsyn$REF %in% c("A", "G", "T", "C"))
bork_nonsyn <- subset(bork_nonsyn, bork_nonsyn$ALT %in% c("A", "G", "T", "C"))
bork_nonsyn <- bork_nonsyn[!bork_nonsyn$INFO1 == "synonymous_variant", -7]

# select non-synonymous SNPs only
bork_snp2 <- subset(bork_snp2, bork_snp2$POS %in% bork_nonsyn$POS)

lppa <- bork_snp2[bork_snp2$INFO == "lppA", ]

find_gene <- function(x){
  y <- bork_snp2[bork_snp2$INFO == x, ]
...
number_of_strains <- lapply(unique(bork_snp2$INFO), find_gene)
...
bork_unique2 <- distinct(bork_snp2, bork_snp2$POS, .keep_all=TRUE)
# subsetting the bork_unique2 data frame to select the gene names corresponding to the snps in each snp block
block_positions <- list()
for (snp_names in listofnames_t){
  gene_blocks <- bork_unique2[bork_unique2$POS %in% snp_names, ]
...
all_genes <- pivot_wider(bork_unique2, names_from = INFO, values_from = POS)
...
# subsetting the bork2 data frame to select the gene names corresponding to the snps in each snp block
block_positions <- list()
for (snp_names in listofnames_t){
  gene_blocks <- bork_unique2[bork_unique2$POS %in% snp_names, ]
  gene_blocks2 <- as.data.frame(gene_blocks$INFO)
...
narl <- bork2[bork2$INFO == "narL", ]
glnq <- bork2[bork2$INFO == "glnQ", ]
glna4 <- bork2[bork2$INFO == "glnA4", ]
...
geneofinterest <- bork_snp2[bork_snp2$INFO == "aceAb", ]
geneofinterest <- bork_snp2[bork_snp2$INFO == "serA1", ]
geneofinterest <- bork_snp2[bork_snp2$INFO == "PE9", ]
# 57298 contains aceAb which is present in 4 strains 212, 374, 639 and 440, not all beijing strains

# get amino acid info
bork_amino <- bork
# select only SNP variants
bork_amino$INFO <- sapply(bork_amino$INFO, `[`, 11)
# get amino acid info
bork_amino <- subset(bork_amino, bork_amino$REF %in% c("A", "G", "T", "C"))
bork_amino <- subset(bork_amino, bork_amino$ALT %in% c("A", "G", "T", "C"))
# select non-synonymous SNPs only
bork_amino <- subset(bork_amino, bork_amino$POS %in% success_nonsyn$POS)

# bork_amino <- bork_nonsyn
bork_amino$INFO4 <- bork_amino$INFO

bork_amino[bork_amino$POS %in% geneofinterest$POS, ]
bork_amino$INFO4 <- gsub("Arg", "R", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("His", "H", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Lys", "K", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Asp", "D", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Glu", "E", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Ser", "S", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Thr", "T", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Asn", "N", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Gln", "Q", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Cys", "C", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Gly", "G", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Pro", "P", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Ala", "A", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Val", "V", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Ile", "I", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Leu", "L", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Met", "M", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Phe", "F", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Tyr", "Y", bork_amino$INFO4)
bork_amino$INFO4 <- gsub("Trp", "W", bork_amino$INFO4)

get_aminoacid <- function(x){
  geneofinterest <- bork_snp2[bork_snp2$INFO == x, ]
  df <- bork_amino[bork_amino$POS %in% geneofinterest$POS, ]
  # df$gene <- x
  print(df)
}
...
# for Nadhira project
rs6112[which(str_detect(rs6112$gene, c("ecc")) == TRUE), 1:14]
whib6_genes <- c("nrd", "pyr", "dfp", "esp", "yid", "lep", "vap", "cyd", "cyp",
                 "fad", "fab", "acp", "kas", "accD", "pps", "cys", "met", "whiB", "nar",
                 "tat", "maz", "prp", "acp", "kas", "des", "sig", "hpr", "rps", "rpl",
                 "rpm", "eccD", "sec", "pir", "pkn", "suc", "lip", "tgs", "acr",
                 "yrb", "msr", "trx", "Rv0091", "Rv0516c", "Rv0338c")
whib6_genes_specific <- c("nrdH", "pyrR", "dfp", "espF", "espJ", "epsK", "epsB", "yidC", "lepB", "vapB31", "vapC10", "vapB10", "cydD", "cyp144",
                          "fadE18", "fabB", "acpM", "kasA","kasB",  "accD6", "ppsE", "cysD", "metC", "whiB", "narU",
                          "nrdF2", "espE", "tatB", "mazF4", "vapB46",  "prpD", "prpC", "acpM", "kasA", "kasB", "desA3", "fas", "sigB", "nrdF1", "nrdF2", "ndkA", "hprT", "rpsL", "rpsJ", "rplD", "rplV", "rplP",
                          "rpmD", "rpmG1", "espL", "eccD3", "secY", "pirG", "vapC36", "vapC38", "sigE", "pknH", "accD3", "sucD", "prpD", "prpC", "lipL", "ppsC", "tgs3", "acrA1",
                          "yrbE4B", "cysO", "cysD", "metA", "metE", "msrA", "trxB1", "Rv0091", "Rv0516c", "Rv0338c", "Rv0526")
met_genes <- c("Rv1403C", "Rv1405c", "metH")

queries <- rs6112 %>% 
  filter(str_detect(rs6112$gene, paste(whib6_genes, collapse = "|")))
queries <- queries[queries$rep_score > 1, ]
queries2 <- rs6112 %>% 
  filter(str_detect(rs6112$gene, paste(whib6_genes_specific, collapse = "|")))
queries3 <- rs6112 %>% 
  filter(str_detect(rs6112$gene, paste(met_genes, collapse = "|")))

queries_final <- queries2[, c(1:2, 5, 7:9, 17)]
# for all the genes
# queries_final2 <- queries[, c(1:2, 5, 7:9, 17)]

write.csv(queries_final, "whib6_genes_thesis.csv")

# use filtered rep score
queries <- rs6112_filtered %>% 
  filter(str_detect(rs6112_filtered$gene, paste(whib6_genes, collapse = "|")))
queries <- queries[queries$rep_score > 1, ]
queries2 <- rs6112_filtered %>% 
  filter(str_detect(rs6112_filtered$gene, paste(whib6_genes_specific, collapse = "|")))
queries3 <- rs6112_filtered %>% 
  filter(str_detect(rs6112_filtered$gene, paste(met_genes, collapse = "|")))

queries_final2 <- queries2[, c(1:2, 5, 7:9, 17)]
# for all the genes
# queries_final2 <- queries[, c(1:2, 5, 7:9, 17)]

write.csv(queries_final2, "whib6_genes_filteredrepscore_ thesis.csv")

rs6112_t <- lapply(rs6112$gene, get_aminoacid)
rs6_t <- tibble(rs6112_t)
rs6_t <- rs6_t %>% unnest()

# for all the genes
# rs6112_aminoacids <- rs6_t[rs6_t$INFO3 %in% queries_final2$gene, ]
rs6112_aminoacids <- rs6_t[rs6_t$INFO3 %in% queries_final$gene, ]
rs6112_aminoacids <- rs6112_aminoacids[!rs6112_aminoacids$INFO2 == "MODIFIER", ]
colnames(rs6112_aminoacids) <- c("position", "ID", "REF_allele", "ALT_allele", "QUAL", "FILTER", "amino_acid_change", "FORMAT", "strain", "gene")
# colnames(rs6112_aminoacids) <- c("position", "ID", "REF_allele", "ALT_allele", "QUAL", "FILTER", "FORMAT", "strain", "variant_type", "effect", "gene", "amino_acid_change_short", "amino_acid_change")
rs6112_aminoacids$amino_acid_change <- gsub("p.", "", rs6112_aminoacids$amino_acid_change)
# rs6112_aminoacids$amino_acid_change_short <- gsub("p.", "", rs6112_aminoacids$amino_acid_change_short)
# rs6112_aminoacids$amino_acid_change <- gsub("p.", "", rs6112_aminoacids$amino_acid_change)
# for genes specific to the paper
write.csv(rs6112_aminoacids, "rs6112_amino_acids2.csv")
# for genes related to the paper
# write.csv(rs6112_aminoacids, "rs6112_allgenes_amino_acids.csv")
# for genes related to the paper, with unique positions
# rs6112_aminoacids_nonduplicated <- rs6112_aminoacids[!duplicated(rs6112_aminoacids$position), ]
# write.csv(rs6112_aminoacids_nonduplicated, "rs6112_allgenes_amino_acids_filtered.csv")

# get the amino acid change for each gene in specific snp blocks
rs79480_t <- lapply(rs79480$gene, get_aminoacid)
rs7_t <- tibble(rs79480_t)
rs7_t <- rs7_t %>% unnest()

# look at snp blocks with only 1 snp
rep_score_only1 <- rep_score[!rep_score$block1_name %in% rep_score_filtered$block1_name, ]
snp_blocks_only1 <- snp_blocks[rownames(snp_blocks) %in% rep_score_only1$block1_name, ]
snp_blocks_only1 <- as.data.frame(t(snp_blocks_only1))
snp_blocks_only1[, 18:19] <- morethan1_block[, 27:28]

write.csv(rep_score_only1[order(rep_score_only1$rep_score, decreasing = TRUE), ], "rep_score_only1snp.csv")
write.csv(snp_blocks_only1, "snp_blocks_only1snp.csv")
...
# get amino acids for Nadhira
# want bork_amino but without non-snp variants
snps_nonsyn_nadhira <- success_nonsyn
index <- match(snps_nonsyn_nadhira$POS, bork_amino$POS)
snps_nonsyn_nadhira$amino_acid <- bork_amino$INFO
write.csv(snps_nonsyn_nadhira, "snps_nonsyn_nadhira.csv")

# get amino acids for Konstanze

setwd("D:/Lineage/Student_projects/")
k_genes <- read.csv("Genes_Konstanze.csv")
k_bork <- bork_amino
index <- match(k_bork$POS, bork2$POS)
k_bork$Gene <- bork2$INFO[index]
k_bork <- k_bork[k_bork$Gene %in% k_genes$Genes, ]

write.csv(k_bork, "Konstanze_SNPs.csv")
...
# select the info you want
bork5 <- bork
bork5$INFO <- sapply(bork5$INFO, `[`, 4)

# keep only single nucleotide variants
bork5 <- subset(bork5, bork5$REF %in% c("A", "G", "T", "C"))
bork5 <- subset(bork5, bork5$ALT %in% c("A", "G", "T", "C"))
# synonymous and non-synonymous as individual objects
# bork5_nonsyn <- subset(bork5, bork5$POS %in% success_nonsyn$POS)

# bork3 <- bork2
bork5$new <- categories$cat[match(bork5$INFO, categories$Name)]
# check func_ann[func_ann$INFO == "Rv3922c", ]

allsnps_functions <- bork5
...
all_lins <- read.csv("global_dataset2.csv")[,1:9]
colnames(all_lins)[1] <- "lineage"
all_lins <- all_lins[, -7]

# Paulina's dataset
# all_lins <- read.csv("global_pos.csv")
# rownames(all_lins) <- all_lins$X
# all_lins <- all_lins[, -1]

# add emtpy columns to dataset
df_lins <- data.frame(matrix(ncol = length(summary(factor(all_lins$lineage))), nrow = dim(all_lins)[1]))
colnames(df_lins) <- unique(all_lins$lineage)
all_lins2 <- cbind(all_lins, df_lins)
all_lins2 <- all_lins2[, -99]

mine <- all_lins2 %>% distinct(POS, .keep_all = TRUE)
mine <- mine$POS

empty_table_ref <- list()
for (position in mine){ #for snp position
  zero_one <- list()
  my_table <- all_lins2[all_lins2$POS == position, ] #grab all rows with this position
  print(my_table)
  lineages_present <- my_table$lineage
  lineages_present <- paste(lineages_present, sep="") #grab all the lineages where this snp is present (rename to fit with names)
  print(lineages_present)
  lineages_absent <- setdiff(x = colnames(all_lins2)[dim(all_lins)[2]+1:(dim(all_lins2)[2]-dim(all_lins)[2])], y = lineages_present) #grab all lineages where this snp is absent
  print(lineages_absent)
  zeros <- rep(0, length(lineages_absent)) #for every lineage where its absent add a zero
  ones <- rep(1, length(lineages_present)) #for every lineage where its present add a one
  zeros_ones <- c(zeros, ones)
  names(zeros_ones) <- c(lineages_absent, lineages_present)
  zeros_ones <- zeros_ones[colnames(all_lins2)[dim(all_lins)[2]+1:(dim(all_lins2)[2]-dim(all_lins)[2])]] #reorders the columns to the order of linesgaesin the original table
  empty_table_ref <- rbind(empty_table_ref, zeros_ones) #row = snp, add each row one at a time
}
rownames(empty_table_ref) <- mine
...
snps_uncommon <- subset(reference, !rownames(reference) %in% rownames(snps_nonsynonymous))
snps_uncommon[, 91:104] <- 0
colnames(snps_uncommon)[91:104] <- colnames(snps_nonsynonymous)
snps_uncommon_again <- subset(snps_nonsynonymous, !rownames(snps_nonsynonymous) %in% rownames(reference))
snps_uncommon_again[, 15:104] <- 0
colnames(snps_uncommon_again)[15:104] <- colnames(reference)
...
bork3 <- bork
bork3$INFO <- sapply(bork3$INFO, `[`, 4)
bork3$new <- func_ann$Pathways.of.gene[match(bork3$INFO, func_ann$Name)]
# check func_ann[func_ann$INFO == "Rv3922c", ]
...
bork3 <- bork
bork3$INFO <- sapply(bork3$INFO, `[`, 4)
bork3$new <- categories$cat[match(bork3$INFO, categories$Name)]

allsnps_functions <- bork3
# allsnps_function <- allsnps_functions[allsnps_functions$POS %in% rownames(snps_all2), ]
...
###-- visualise the differentially abundant metabolites IGNORE #####

# metabolite levels
log_avg2 <- as.data.frame(log_avg[, -15])
d <- as.data.frame(t(log_avg))
d$strain <- c(condition, "H37Rv")
d$lineage <- c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2", "lin4") # check
order_get <- order(match(d$strain, order_lin))
d <- d[order_get, ]
sd_avg2 <- as.data.frame(sd_avg[, -15])
sd <- as.data.frame(t(sd_avg2))
sd$strain <- c(condition)
sd$lineage <- c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2") # check
order_get <- order(match(sd$strain, order_lin))
sd <- sd[order_get, ]

glycolysis <- d[, c(1:9, 42)]
aminoacids <- d[, c(14:25, 42)]
urea <- d[, c(28:31, 42)]
nucleotides <- d[, c(32:36, 42)]
peptidoglycan <- d[, c(38:40, 42)]
targeted_bar <- melt(d, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

targeted_bar <- melt(glycolysis, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

targeted_bar <- melt(aminoacids, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

targeted_bar <- melt(urea, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

targeted_bar <- melt(nucleotides, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

targeted_bar <- melt(peptidoglycan, id.vars = "lineage")
ggplot(targeted_bar, aes(fill = variable, x = lineage, y = value)) + geom_bar(stat = "identity", position = "fill")

# single metabolites
ggplot(d, aes(x = reorder(as.character(strain), 1:14), y = d[,1], fill = lineage)) + geom_bar(stat = "identity")
ggplot(d, aes(x = reorder(as.character(strain), 1:15), y = d[,37], fill = lineage)) + geom_bar(stat = "identity")
ggplot(d, aes(x = reorder(as.character(strain), 1:15), y = d[,3], fill = lineage)) + geom_bar(stat = "identity")
ggplot(d, aes(x = reorder(as.character(strain), 1:15), y = d[,17], fill = lineage)) + geom_bar(stat = "identity")
ggplot(d, aes(x = reorder(as.character(strain), 1:15), y = d[,29], fill = lineage)) + geom_bar(stat = "identity")
ggplot(d, aes(x = reorder(as.character(strain), 1:15), y = d[,34], fill = lineage)) + geom_bar(stat = "identity")

rownames(d) <- gsub("X", "s", rownames(d))
d$sequence <- order(match(rownames(d), rownames(morethan1_block)))
d$sublineage <- c(as.character(morethan1_block$`28`), "lin4_sub")
ggplot(d, aes(x = reorder(as.character(strain), sequence), y = Ergothioneine, fill = lineage)) + geom_bar(stat = "identity")
...
X <- list(SNP = t(snp_blocks),
          metab_tar = t(log_avg2))
Y <- factor(c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2"))
# strains <- morethan1_block$`28`
# Y <- factor(strains)
# Y <- factor(colnames(log_avg2))
list.keepX <- list(SNP = c(116,116), metab_tar = c(40,40))
...
png("diablo_circplot_targeted_blocks_strains_thesis.png", height = 1750, width = 1750, res = 175)
circ <- circosPlot(res.diablo, cutoff=0.75, showIntraLinks=FALSE, comp=1, size.variables=0.5, size.legend = 1.29, line = FALSE,
                   color.cor = c("#3388BB", "black"),
                   color.blocks = c("#CC6666", "gray67"))
dev.off()

png("diablo_circplot_targeted_component2_strains_thesis.png", height = 1750, width = 1750, res = 175)
...
##### pca cluster based on significant untargeted metabolites #####
...
col_lineages <- string.to.colors(c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2", "lin4"))

# pca plot
data <- nlog_avg
data <- data[rownames(data) %in% rownames(sig_res_neg), ]
data <- t(data)
pca <- prcomp(data)

setwd("D:/Lineage/Data_run/results/")

png("pca_signegmetab_untargeted_neg_thesis.png", width = 500, height = 500)

autoplot(pca, data=data, label=TRUE, shape=FALSE, label.size=6,
         colour=c("#999999", "#003366", "#999999", "#CC6666", "#CC6666", "#003366", "#003366", "#999999", "#CC6666", "#CC6666", "#999999", "#003366", "#003366", "#999999", "#003366")) +
  theme_bw() +
  theme(axis.title.x = element_text(size=25, margin=margin(7, 1, 1, 1)),
        axis.title.y = element_text(size=25, margin=margin(1, 7, 1, 1)),
...
data <- plog_avg
data <- data[rownames(data) %in% rownames(sig_res_pos),]
data <- t(data)
pca <- prcomp(data)

png("pca_signegmetab_untargeted_pos_thesis.png", width = 500, height = 500)
...
# untargeted metabolite plots

correlation_untargeted_neg <- rbind(snps_noncorrelated, nlog_avg2)
cor_matrix <- cor(t(correlation_untargeted_neg))

data.df <- rbind(snps_noncorrelated, plog_avg2)
cor_matrix <- cor(t(data.df))

# select the info you want
bork2 <- bork
bork2$INFO <- sapply(bork2$INFO, `[`, 4)

# specific nsSNP blocks
snps2 <- snps_nonsynonymous
rownames(snps2) <- gsub("s", "", rownames(snps2))
snps3 <- snps2[rownames(snps2) %in% c(1849, 6112, 8452, 10986, 11820),]
snps4 <- snps2[rownames(snps2) %in% c(13100, 13275),]
snps5 <- snps2[rownames(snps2) %in% c(c(14861, 15117, 16119)),]
selected <- rbind(snps3, snps4, snps5)

snps3 <- snps2[rownames(snps2) %in% c(1849, 6112),]
snps5 <- snps2[rownames(snps2) %in% c(15117),]
selected_noncorrelated <- rbind(snps3, snps5)

selected_info <- bork[which(bork$POS %in% rownames(selected)), ]
selected_info$INFO <- sapply(selected_info$INFO, `[`, 11)
selected_info <- selected_info[c(1:9, 11),]
...
plog_avg2 <- plog_avg2[,1:14]
colnames(nlog_avg2) <- gsub("X", "s", colnames(nlog_avg2))
rownames(nlog_avg2) <- gsub("X", "m", rownames(nlog_avg2))
colnames(plog_avg2) <- gsub("X", "s", colnames(plog_avg2))
rownames(plog_avg2) <- gsub("X", "m", rownames(plog_avg2))

X <- list(SNP = t(snp_blocks),
          metab_neg = t(nlog_avg2))
Y <- factor(c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2"))
list.keepX <- list(SNP = c(116,116), metab_neg = c(150,150))
res.diablo <- block.splsda(X, Y, keepX=list.keepX)
vars_neg <- selectVar(res.diablo) # check variables selected
plotIndiv(res.diablo)
plotVar(res.diablo, var.names=c(FALSE, FALSE), legend=TRUE, style="lattice") # plot=FALSE gives matrix
plotDiablo(res.diablo, ncomp = 1)
plotLoadings(res.diablo, comp = 1, contrib = "max")
...
png("diablo_circplot_untargeted_neg_blocks_thesis.png", height = 1200, width = 1300)
circ <- circosPlot(res.diablo, cutoff=0.75, showIntraLinks=FALSE, comp = 1, size.variables=0.9, size.labels = 1.75, size.legend = 2.75, line = TRUE,
                   color.cor = c("#3388BB", "black"),
                   color.blocks = c("#CC6666", "gray67"),
                   color.Y = c("#CC6666", "gray67", "#003366"))
dev.off()

png("diablo_circplot_untargeted_neg_component2_thesis.png", height = 1200, width = 1300)
circ <- circosPlot(res.diablo, cutoff=0.75, showIntraLinks=FALSE, comp = 2, size.variables=0.9, size.labels = 1.75, size.legend = 2.75, line = TRUE,
                   color.cor = c("#3388BB", "black"),
                   color.blocks = c("#CC6666", "gray67"),
                   color.Y = c("#CC6666", "gray67", "#003366"))
dev.off()

colnames(plog_avg2) <- colnames(snps_nonsynonymous)
X <- list(SNP = t(snp_blocks),
          metab_pos = t(plog_avg2))
Y <- factor(c("lin2", "lin4", "lin2", "lin1", "lin1", "lin4", "lin4", "lin2", "lin1", "lin1", "lin2", "lin4", "lin4", "lin2"))
list.keepX <- list(SNP = c(116,116), metab_pos = c(150,150))
res.diablo <- block.splsda(X, Y, keepX=list.keepX)
vars_pos <- selectVar(res.diablo) # check variables selected
plotIndiv(res.diablo)
plotVar(res.diablo, var.names=c(FALSE, FALSE), legend=TRUE, style="lattice") # plot=FALSE gives matrix
plotDiablo(res.diablo, ncomp = 1)

png("diablo_circplot_untargeted_pos_blocks_thesis.png", height = 1200, width = 1300)
circ <- circosPlot(res.diablo, cutoff=0.85, showIntraLinks=FALSE, comp = 1, size.variables=0.9, size.labels = 1.75, size.legend = 2.75, line = TRUE,
                   color.cor = c("#3388BB", "black"),
                   color.blocks = c("#CC6666", "gray67"),
                   color.Y = c("#CC6666", "gray67", "#003366"))
dev.off()

png("diablo_circplot_untargeted_pos_component2_thesis.png", height = 1200, width = 1300)
circ <- circosPlot(res.diablo, cutoff=0.8, showIntraLinks=FALSE, comp=2, size.variables=0.9, size.labels = 1.75, size.legend = 2.75, line = TRUE,
                   color.cor = c("#3388BB", "black"),
                   color.blocks = c("#CC6666", "gray67"),
                   color.Y = c("#CC6666", "gray67", "#003366"))
dev.off()
...
# figure 2 is this order

# figure 3 is this order
# Doubling_times_paper, pca_targeted_thesis_repeat_2023
...
##### global plot for thesis corrections #####

# loads all files in current directory ending with .csv
setwd("D:/ENA_dataset/run_files/Final/VCF/Final/Final/")
...
# nsSNP blocks with more than 1 gene being affected
# 368092
# snp_blocks_genes <- rowSums(snp_blocks) > 1
# summary(rowSums(snp_blocks) > 1)
# write.csv(snp_blocks_genes, "snp_blocks_morethan1gene.csv")

# correlation plot of nsSNPs
cor_nssnps <- rcorr(as.matrix(t(snps_nonsynonymous)))

# filter for p-value above 0.05
cor_snps <- as.data.frame(cor_nssnps$P)
cor_snps$snp_names <- as.character(rownames(cor_snps))
cor_snps <- melt(cor_snps, "snp_names")
length(unique(cor_snps$variable)) # number of snps
nssnp_p <- cor_snps[cor_snps$value < 0.05, ]
length(unique(nssnp_p$variable)) # number of snps with significant correlation
nssnp_cor_filtered <- unique(nssnp_p$variable)

nssnps_correlated <- rbind(snps_nonsynonymous[rownames(snps_nonsynonymous) %in% nssnp_cor_filtered, ])
nssnps_correlated <- rcorr(as.matrix(t(nssnps_correlated)))

jpeg("correlationplot_nssnps_thesis.jpeg", height = 2700, width = 4500, res = 100)
corrplot(nssnps_correlated$r[100:150, 100:150], p.mat = nssnps_correlated$P[100:150, 100:150], sig.level = 0.05, insig = "blank", outline = TRUE, type = "upper",
         order = "hclust",  tl.col = "black", addgrid.col = NA, col = brewer.pal(n = 8, name = "RdBu"),
         mar = c(0.5, 1, 0.5, 1),
         tl.cex = 1.5,
         cl.cex = 1.5,
...
# filter for correlation of > 0.98 or < -0.98
cor_snps <- as.data.frame(nssnps_correlated$r)
cor_snps$snp_names <- as.character(rownames(cor_snps))
cor_snps <- melt(cor_snps, "snp_names")
length(unique(cor_snps$variable)) # number of snps
nssnp_r <- cor_snps[cor_snps$value == 0.99, ]
length(unique(nssnp_r$variable))
nssnp_r2 <- cor_snps[cor_snps$value < -0.99, ]
length(unique(nssnp_r2$variable))
nssnp_r <- rbind(nssnp_r, nssnp_r2)
length(unique(nssnp_r$variable)) # number of snps with significant correlation
nssnp_cor_filtered <- unique(nssnp_r$variable)

nssnps_correlated <- rbind(snps_nonsynonymous[rownames(snps_nonsynonymous) %in% nssnp_cor_filtered, ])
nssnps_correlated <- rcorr(as.matrix(t(nssnps_correlated)))

jpeg("correlationplot_nssnps_thesis.jpeg", height = 2700, width = 4500, res = 100)
corrplot(nssnps_correlated$r[1:100, 1:100], p.mat = nssnps_correlated$P[1:100, 1:100], sig.level = 0.05, insig = "blank", outline = TRUE, type = "upper",
         order = "hclust",  tl.col = "black", addgrid.col = NA, col = brewer.pal(n = 8, name = "RdBu"),
         mar = c(0.5, 1, 0.5, 1),
         tl.cex = 1.5,
         cl.cex = 1.5,
...
# get 6112 correlations
block_6112 <- as.data.frame(nssnps_correlated$r[, "6112"])
colnames(block_6112) <- "correlation_value"
# select highly correlated values with 6112
correlated_block6112_1 <- subset(block_6112, block_6112$correlation_value > 0.99)
correlated_block6112_2 <- subset(block_6112, block_6112$correlation_value < -0.99)
correlated_block6112 <- rbind(correlated_block6112_1, correlated_block6112_2)
dim(correlated_block6112)
write.csv(correlated_block6112, "rs6112_allnssnps.csv")

# find lineage 4 nsSNP block

snp_blocks_lin4 <- snp_blocks[, c(12:13, 2, 6:7)]
snp_blocks_lin4 <- snp_blocks_lin4[which(rowSums(snp_blocks_lin4) == 0), ]
snp_blocks_lin12 <- snp_blocks[, c(1, 3:5, 8:11, 14)]
snp_blocks_lin12 <- snp_blocks_lin12[which(rowSums(snp_blocks_lin12) == 9), ]
which(rownames(snp_blocks_lin12) %in% rownames(snp_blocks_lin4))

snp_blocks_lin2 <- snp_blocks[, c(1, 3, 8, 11, 13)]
snp_blocks_lin4 <- snp_blocks_lin4[which(rowSums(snp_blocks_lin4) == 0), ]

# get the genes which contain these nsSNPs

# altnertavive method of getting the snps in block rs6112
snp_block_6112 <- as.data.frame(cor_nssnps$r[, "6112"])
colnames(snp_block_6112) <- "correlation_value"
snp_block_6112 <- subset(snp_block_6112, snp_block_6112$correlation_value > 0.99)
